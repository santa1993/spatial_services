...
def long_lat():
	...
	q1 = '''SELECT AsGeoJSON(Transform(a.geometry, 4326)) as geometry, a.name, a.node_id
...
@app.route('/allMarkers', methods=['POST', 'GET'])
def allMarkers():
	conn1 = g.db.cursor()
	...
	q1 = '''SELECT AsGeoJSON(Transform(a.geometry, 4326)) as geometry, a.name, a.node_id
        FROM shops_tuebingen
        AS a 
        WHERE Contains(Buffer(GeomFromText('POINT(%s %s)', 3857), %s), a.geometry) AND name IN ("Aldi", "Rewe", "Netto", "Edeka") AND a.ROWID 
        IN (SELECT ROWID FROM SpatialIndex WHERE f_table_name = 'shops_tuebingen' 
